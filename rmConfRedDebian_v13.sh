...
#
# rmConfRedDebian_v13.sh - v250924-0853
#
# rmCMD=rmConfRedDebian_v13.sh && bash -c "$(curl -fsSL https://github.com/ricardomonla/RM-rmCMDs/raw/refs/heads/main/${rmCMD})"

rmCMD="rmConfRedDebian_v13.sh"

cat << 'SHELL' > "${rmCMD}"
#!/usr/bin/env bash
# ==============================================================
# Script: rmConfRedDebian_v13.sh
# Autor: Lic. Ricardo MONLA (https://github.com/ricardomonla)
# Versión: v250924-0853
...
submenu_config() {
  ...
  while true; do
    ...
    if [ "$mode" = "DHCP" ]; then
      ...
      OPC=${OPC:-4}
      case $OPC in
        1) mode="STATIC" ;; # cambiar modo
        3) guardar_config "$IFACE" "$mode" "$ip" "$dnsline" ;;
        4) return ;;
