...
#
# rmConfRedDebian_v10.sh - v250923-1824
#
# rmCMD=rmConfRedDebian_v10.sh && bash -c "$(curl -fsSL https://github.com/ricardomonla/RM-rmCMDs/raw/refs/heads/main/${rmCMD})"

rmCMD="rmConfRedDebian_v10.sh"

cat << 'SHELL' > "${rmCMD}"
#!/usr/bin/env bash
# ==============================================================
# Script: rmConfRedDebian_v10.sh
    ...
    if [ "$mode" = "DHCP" ]; then
      ...
      case $OPC in
        1|2)
          ...
          ;;
        3) guardar_config "$IFACE" "$mode" "$ip" "$dnsline"; return ;;
        4) return ;;
      ...
      case $OPC in
        ...
        4) guardar_config "$IFACE" "$mode" "$ip" "$dnsline"; return ;;
        5) return ;;
        *) echo -e "${RED}❌ Opción inválida.${RESET}" ;;
      esac
    fi
